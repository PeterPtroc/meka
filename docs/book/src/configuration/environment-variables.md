...
| `AGSH_INSTRUCTIONS` | Replace `[prompt].instructions` for this run. Equivalent to `--instructions`. Used by the `agbox` container wrapper to tell the agent it can install packages freely. | `Be terse.` |
